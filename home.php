...
<head>
    ...
    <!-- Include jQuery for simplicity to show slide down conrats message, and for classes list in script2.js -->
...
<body>
    <div class="wrapper">
      ...
      if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) { echo htmlspecialchars($row['date']); }
    ?>">
                        ...
                        <select name="class_id" id="class_id" required>
                            ...
                            <?php
        $stmt = $conn->prepare("SELECT c_id, className FROM Classes WHERE user_id = ?");
        $stmt->execute([$_SESSION['loggedinID']]);
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            echo '<option value="' . htmlspecialchars($row['c_id']) . '">' . htmlspecialchars($row['className']) . '</option>';
        }
      ?>
                        </select>
                        <br>
                    ...
                    <!-- Tasks list container (will be populated and grouped by class) -->
                    <ul class="tasksList"></ul>
                </div>

                <!-- Output a JavaScript mapping of class id to class name -->
                <script>
                var classMapping = <?php 
      $stmt = $conn->prepare("SELECT c_id, className FROM Classes WHERE user_id = ?");
      $stmt->execute([$_SESSION['loggedinID']]);
      echo json_encode($stmt->fetchAll(PDO::FETCH_ASSOC));
    ?>;
                var classMap = {};
                classMapping.forEach(function(cls) {
                    classMap[cls.c_id] = cls.className;
                });
                </script>
                ...
                <div class="classes-section">
                    ...
                    <form id="classesForm">
                        ...
                        <textarea id="classNotes" name="classNotes" placeholder="Enter class notes"></textarea>

                        <button type="submit">Submit</button>
                    </form>
                    <ul class="classesList">
                        <!-- The list of classes will be dynamically loaded here -->
                    </ul>
                    <br><br>
                    <span>Note: <code style="color: red;">Deleting a class</code> will delete all related
                        tasks.</span>
                </div>
    ...
    ?>
                    ...
                    <div class="holidays-section">
                        ...
                        <div id="holidayFormContainer" style="display: none;">
                            <form id="holidayForm">
                                ...
                                <label>
                                    <input type="checkbox" id="holidayClasses" name="holidayClasses">
                                    Applies to Classes
                                </label>
        ...
        <footer>
            <p>ENSE 400 - ENSE 477 Capstone © 2024</p>
        </footer>
    </div>
